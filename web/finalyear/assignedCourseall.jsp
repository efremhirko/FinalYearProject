		...
		<div class="pd-ltr-20 xs-pd-20-10">
			<div class="min-height-200px">
				...
				<div class="card-box mb-30">
					<div class="pd-20">
						<h4 class="text-blue h4">The Distibuted Course Information</h4>
					</div>
					<div class="pb-20">
     ...
     <thead>
          ...
          <tbody>
            ...
            <% if (sessionObj != null && sessionObj.getAttribute("dpt") != null) { 
                  ...
                  switch (dpt) {
                    case "SE":
                      query = "SELECT * FROM sweassignedcourse";
                      break;
                    case "CS":
                      query = "SELECT * FROM csassignedcourse";
                      break;
                    case "IS":
                      query = "SELECT * FROM isassignedcourse";
                      break;
                    case "ISY":
                      query = "SELECT * FROM isyassignedcourse";
                      break;
                    case "IT":
                      query = "SELECT * FROM itassignedcourse";
                      break;
                    default:
                      query = "SELECT * FROM cseassignedcourse";
                      break;
                  }
        ...
        <% 
          try {
            ...
            switch ((String) sessionObj.getAttribute("dpt")) {
              case "SE":
                ResultSet rsSE = stmt.executeQuery("SELECT COUNT(*) AS total FROM sweassignedcourse");
                rsSE.next();
                totalRecords = rsSE.getInt("total");
                break;
              case "CS":
                ResultSet rsCS = stmt.executeQuery("SELECT COUNT(*) AS total FROM csassignedcourse");
                rsCS.next();
                totalRecords = rsCS.getInt("total");
                break;
              case "IS":
                ResultSet rsIS = stmt.executeQuery("SELECT COUNT(*) AS total FROM isassignedcourse");
                rsIS.next();
                totalRecords = rsIS.getInt("total");
                break;
              case "ISY":
                ResultSet rsISY = stmt.executeQuery("SELECT COUNT(*) AS total FROM isyassignedcourse");
                rsISY.next();
                totalRecords = rsISY.getInt("total");
                break;
              case "IT":
                ResultSet rsIT = stmt.executeQuery("SELECT COUNT(*) AS total FROM itassignedcourse");
                rsIT.next();
                totalRecords = rsIT.getInt("total");
                break;
              default:
                ResultSet rsDefault = stmt.executeQuery("SELECT COUNT(*) AS total FROM cseassignedcourse");
                rsDefault.next();
